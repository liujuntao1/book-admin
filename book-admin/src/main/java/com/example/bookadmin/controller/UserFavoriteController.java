...
public class UserFavoriteController {
    ...
    // 添加收藏
    @PostMapping
    public CommonResult<String> add(@RequestBody UserFavoriteBook favorite) {
        ...
        if (existing != null) {
            return CommonResult.success("已收藏，无需重复操作");
        }
    ...
    // 取消收藏
    @DeleteMapping
    public CommonResult<String> remove(@RequestParam Long userId, @RequestParam Long bookId) {
        LambdaQueryWrapper<UserFavoriteBook> wrapper = new LambdaQueryWrapper<>();
        ...
        return CommonResult.success(removed ? "取消成功" : "未找到收藏记录");
    }

    // 查询是否收藏（用于前端按钮状态）
    @GetMapping("/is-favorited")
    public CommonResult<Boolean> isFavorited(@RequestParam Long userId, @RequestParam Long bookId) {
        LambdaQueryWrapper<UserFavoriteBook> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(UserFavoriteBook::getUserId, userId)
                .eq(UserFavoriteBook::getBookId, bookId);
    ...
    public CommonResult<?> queryFavoritedList(@RequestParam(required = false, defaultValue = "1") Integer page,
